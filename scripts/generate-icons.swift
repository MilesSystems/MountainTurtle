...
let overlay = output.appendingPathComponent("icon-overlay", isDirectory: true)
...
try finderMetadata(flags: 0x0400).write(to: overlay.appendingPathComponent("._."))
try finderMetadata(flags: 0x4000).write(to: overlay.appendingPathComponent("._.VolumeIcon.icns"))
